...
export class PriceComponent implements OnInit, OnDestroy {
  ...
  updatePrices(newPrices: Price[]) {
    for (const newPrice of newPrices) {
      const foundPrice = this.prices.find(
        price =>
  ...
  getPrice() {
    ...
    this.priceService
      .getPrice(this.source, this.addedCurrency.currency)
      .then(price => {
        this.addedCurrency.high = this.addedCurrency.low = price.last.toFixed(
          8
        );
