...
namespace CurrencyTrackerServer.Web.Controllers
{
    ...
    public class ChangesController : Controller
    {
        ...
        private readonly BittrexApiDataSource _bittrexApi;
        private readonly PoloniexApiDataSource _poloniexApi;
        ...
        public ChangesController(BittrexTimerWorker bWorker, PoloniexTimerWorker pWorker,
          ...
          PoloniexApiDataSource poloniexApiDataSource, UserContainersManager userContainersManager,
           BittrexApiDataSource bittrexApi, PoloniexApiDataSource poloniexApi)
        {
            _bWorker = bWorker;
            _pWorker = pWorker;
            ...
            this._bittrexApi = bittrexApi;
            this._poloniexApi = poloniexApi;
            _bWorker.Start();
            _pWorker.Start();
        }
        ...
        {
            IEnumerable<CurrencyChangeApiData> changes;
            switch (source)
            {
                case UpdateSource.Bittrex:
                    changes = await _bittrexApi.GetData();
                    break;
                case UpdateSource.Poloniex:
                    changes = await _poloniexApi.GetData();
                    break;
                default:
                    return Array.Empty<ChangePercentageDto>();
            }

            return changes.Select(c => new ChangePercentageDto
            { Currency = c.Currency, PercentChanged = Math.Round(c.PercentChanged, 2) });
        }
